...
from chromhandler.fitting.shift import align_chromatogram_shifts, shift_trace_linear
...
def test_shift_trace_linear_uses_continuous_interpolation() -> None:
    ...
    shifted_neg = np.asarray(
        shift_trace_linear(trace, np.array(-1.4, dtype=np.float32))
    )

    np.testing.assert_allclose(
        shifted_pos, np.array([0.0, 4.0, 14.0, 24.0], dtype=np.float32), atol=1e-6
    )
    np.testing.assert_allclose(
        shifted_neg, np.array([14.0, 24.0, 30.0, 30.0], dtype=np.float32), atol=1e-6
    )
...
def test_alignment_can_return_fractional_shifts() -> None:
    ...
    shifted = np.interp(x - 0.35, x, base, left=base[0], right=base[-1]).astype(
        np.float32
    )
...
def test_alignment_initializer_detects_large_coarse_shift() -> None:
    ...
    shifted = np.interp(x - 8.0, x, base, left=base[0], right=base[-1]).astype(
        np.float32
    )
    signal = np.stack([base, shifted], axis=0)
